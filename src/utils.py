...
def save_run_config(config_path: Path, args: argparse.Namespace, base_dir: Path, expr_dim: int, resume_path: str):
    ...
    cfg.update({
        ...
        "seed": args.seed,
        "train_promoter_file": str(base_dir / "data" / "processed" / "promoter_train.csv"),
        "val_promoter_file": str(base_dir / "data" / "processed" / "promoter_val.csv"),
        "scrna_file": str(base_dir / "data" / "processed" / "integrated_data.h5ad"),
...
def plot_pred_scatter(model, data_loader, max_steps=200):
...
def plot_loss_curves_from_logfile(log_file: Path):
